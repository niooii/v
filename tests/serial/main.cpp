...
int main()
{
    ...
    // Test basic struct serialization round-trip
    {
        ...
        tctx.assert_now(deserialized.integer == original.integer, "Integer round-trip correct");
        tctx.assert_now(std::abs(deserialized.floating - original.floating) < 0.00001,
    ...
    // Test struct with vector data
    {
        ...
        NestedData deserialized = NestedData::parse(
            reinterpret_cast<const u8*>(bytes.data()), bytes.size());

        tctx.assert_now(deserialized.basic.integer == original.basic.integer,
                        "Nested integer round-trip correct");
        tctx.assert_now(deserialized.basic.text == original.basic.text,
                        "Nested string round-trip correct");
        tctx.assert_now(deserialized.numbers.size() == original.numbers.size(),
                        "Vector size preserved");

        for (size_t i = 0; i < original.numbers.size(); ++i) {
            tctx.assert_now(deserialized.numbers[i] == original.numbers[i],
    ...
    // Test large data
    {
        ...
        tctx.assert_now(bytes.size() > 1000, "Large data produces substantial byte array");

        TestData deserialized = TestData::parse(
            reinterpret_cast<const u8*>(bytes.data()), bytes.size());

        tctx.assert_now(deserialized.integer == large_data.integer,
                        "Large data integer round-trip correct");
        tctx.assert_now(deserialized.text == large_data.text,
                        "Large text round-trip correct");
    }
