...
namespace v {
    ...
    void DefaultRenderDomain::add_render_tasks(daxa::TaskGraph& graph)
    {
        ...
        graph.add_task({
            .attachments = {
                daxa::inl_attachment(
                    daxa::TaskAccessConsts::COLOR_ATTACHMENT,
                    daxa::ImageViewType::REGULAR_2D,
                    swapchain_image),
            },
            .task = [swapchain_image](daxa::TaskInterface ti)
            {
                auto const size = ti.device.info(ti.get(swapchain_image).ids[0]).value().size;
